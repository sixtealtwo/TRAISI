...
export class TestSurveyComponent implements OnInit, AfterViewInit {
	...
	loadIndivCodeCount() {
		this.surveyExecuteService.totalSurveyShortCodes(this.surveyId, 'test').subscribe(result => {
			this.totalIndivCodes = result;
		});
	}

	loadGroupCodeCount() {
		this.surveyExecuteService.totalSurveyGroupCodes(this.surveyId, 'test').subscribe(result => {
