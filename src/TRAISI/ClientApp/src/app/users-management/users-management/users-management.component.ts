...
export class UsersManagementComponent implements OnInit, AfterViewInit {
	...
	loadData() {
		...
		if (this.canViewRoles) {
			...
			this.userGroupService.listUserGroups().subscribe(userGroups =>
				this.onDataLoadSuccessful(results[0], results[1], userGroups), error => this.onDataLoadFailed(error));
			}, error => this.onDataLoadFailed(error));
		}
		else {
			this.accountService.getSoloUsers().subscribe(users => {
			this.userGroupService.listUserGroups().subscribe(userGroups =>
				this.onDataLoadSuccessful(users, this.accountService.currentUser.roles.map(x => new Role(x)), userGroups),
				error => this.onDataLoadFailed(error));
			}, error => this.onDataLoadFailed(error));
		}
		}
	...
	onSearchChanged(value: string) {
		if (this.groupBeingViewed) {
			this.groupUserRows = this.groupUserRowsCache.filter(
				r => Utilities.searchArray(value, false, r.userName, r.user.fullName, r.user.email, r.user.jobTitle, r.user.roles));
		}
		else {
		this.soloUserRows = this.soloUserRowsCache.filter(
			r => Utilities.searchArray(value, false, r.userName, r.fullName, r.email, r.jobTitle, r.roles));
		}
	}
	...
	deleteGroupUser(row: GroupMember) {
		this.alertService.showDialog('Are you sure you want to delete \"' + row.user.userName + '\"?',
										DialogType.confirm, () => this.deleteGroupUserHelper(row));
	}
