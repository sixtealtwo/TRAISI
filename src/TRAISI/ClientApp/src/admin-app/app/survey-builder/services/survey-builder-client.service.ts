...
export class SurveyBuilderClient {
    ...
    addConditionalOperator(surveyId: number, questionPartViewId: number, conditional: QuestionConditionalOperator | null): Observable<FileResponse> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processAddConditionalOperator(<any>response_);
                } catch (e) {
                    return <Observable<FileResponse>><any>_observableThrow(e);
                }
            } else
    ...
    protected processAddConditionalOperator(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
    ...
    deleteConditionalOperator(surveyId: number, questionPartViewId: number, conditional: QuestionConditionalOperator | null): Observable<FileResponse> {
        let url_ = this.baseUrl + "/api/SurveyBuilder/{surveyId}/conditionals?";
        if (surveyId === undefined || surveyId === null)
            throw new Error("The parameter 'surveyId' must be defined.");
        url_ = url_.replace("{surveyId}", encodeURIComponent("" + surveyId)); 
        ...
        return this.http.request("delete", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processDeleteConditionalOperator(<any>response_);
                } catch (e) {
                    return <Observable<FileResponse>><any>_observableThrow(e);
                }
            } else
    ...
    protected processDeleteConditionalOperator(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
...
export enum QuestionOptionValueType {
    String = 0,
    Integer = 1,
    Decimal = 2,
    Boolean = 3,
