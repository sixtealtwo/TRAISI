...
export class BootstrapSelectDirective implements OnInit, OnDestroy {
	...
	constructor(private el: ElementRef) {
		this.changedSubscription = observableFromEvent($(this.el.nativeElement), 'changed.bs.select').subscribe((e: any) =>
			setTimeout(() => this.ngModelChange.emit(this.selected)));
		this.shownSubscription = observableFromEvent($(this.el.nativeElement), 'shown.bs.select').subscribe((e: any) =>
			setTimeout(() => this.shown.emit()));
		this.hiddenSubscription = observableFromEvent($(this.el.nativeElement), 'hidden.bs.select').subscribe((e: any) =>
			setTimeout(() => this.hidden.emit()));
	}
