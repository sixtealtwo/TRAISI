...
export class SurveyBuilderClient {
    ...
    protected processUpdateSurveyLogic(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
    ...
    getSurveyLogic(surveyId: number): Observable<SurveyLogicViewModel[]> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                ...
                return <Observable<SurveyLogicViewModel[]>><any>_observableThrow(response_);
        }));
    }
    ...
    protected processGetSurveyLogic(response: HttpResponseBase): Observable<SurveyLogicViewModel[]> {
        ...
        if (status === 200) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
            let result200: any = null;
            result200 = _responseText === "" ? null : <SurveyLogicViewModel[]>JSON.parse(_responseText, this.jsonParseReviver);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
